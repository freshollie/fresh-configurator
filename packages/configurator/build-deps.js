...
const { dependencies, devDependencies, name: workspaceName } = require('./package.json');
...
(async () => {
  ...
  const workspace = pnp.getDependencyTreeRoots().find(({name}) => name === workspaceName);

  const modulesToBuild = findDeps(workspace, dependencies, REBUILD_MODULES);
  const results = await Promise.all(modulesToBuild.map(([name, folder]) => {
